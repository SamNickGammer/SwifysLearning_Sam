import React from 'react';
import { Link } from 'react-router-dom';
import SampleImage from '../Image/SampleSeal.jpg';

const imageStyle = {
  height: 130,
  width: 130,
  marginRight: 30,
};
const mainDivFlex = {
  display: 'flex',
};
const positionDiv = {
  width: '50%',
  //   position: 'relative',
  //   left: '14%',
  marginBottom: '10px',
  borderRadius: '20px',
  boxShadow:
    'rgb(50 50 93 / 25%) 0px 10px 91px -13px, rgb(0 0 0 / 30%) 0px 20px 60px -30px, rgb(10 37 64 / 35%) 0px -2px 6px 0px inset',
  /* box-shadow: 0 5px 10px rgb(154 160 185 / 5%), 0 15px 40px rgb(0 0 0 / 10%); */
  padding: '3rem',
};

export default function CardBlog(props) {
  const { item } = props;
  return (
    <div style={positionDiv}>
      <Link to={{ pathname: `item/${item.id}`, state: { item: props } }}>
        <div style={mainDivFlex}>
          <img src={SampleImage} alt="" style={imageStyle} />
          <div>
            <h3>{item.title.toString().substring(0, 40)}</h3>
            <h4>{item.body.toString().substring(0, 60)}</h4>
          </div>
          {/* {item.map((item) => {
        return <pre>{item.title}</pre>;
      })} */}
        </div>
      </Link>
    </div>
  );
}
